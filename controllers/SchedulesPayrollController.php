<?php
require_once './models/SchedulesPayroll.php';
require_once './responses/JsonResponse.php';

class SchedulesPayrollController {
    
    public function getSchedulesPayroll() {
        ...
        if ($terms === false) {
            JsonResponse::error('academic_term contiene valores inválidos', 400);
        }
        
        // Instanciar modelo y pasar los parámetros
        $model = new SchedulesPayroll();
        $schedules_Payroll = $model->getSchedulesPayroll($years, $terms);
        
        if ($schedules_Payroll === null) {
            JsonResponse::error('Error al conectar con la base de datos', 500);
        }
        
        JsonResponse::success($schedules_Payroll, 'Horarios carga horaria SICAVP obtenidos correctamente');
    }
